...
class _RefundRequestsScreenState extends ConsumerState<RefundRequestsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await api.get(AppConstants.refundRequestsEndpoint);
      final raw = res.data;
      final list = raw is List ? raw : (raw is Map && raw['results'] is List ? raw['results'] as List : <dynamic>[]);
  ...
  Future<void> _respond(Map<String, dynamic> request, String action, String role) async {
                ...
                Expanded(
                  child: FilledButton(
                    ...
                    style: FilledButton.styleFrom(
                      backgroundColor: action == 'approve' ? AppTheme.success : action == 'escalate' ? AppTheme.warning : AppTheme.error,
                    ),
                    child: Text(action == 'approve' ? 'Approve' : action == 'escalate' ? 'Escalate' : 'Reject'),
                  ),
                ),
              ],
            ),
    ...
    try {
      final api = ApiService();
      await api.post(
        '${AppConstants.refundRequestsEndpoint}${request['id']}/respond/',
              ...
              : _requests.isEmpty
                  ? const Center(
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        SizedBox(height: 12),
                        Text('No refund requests', style: TextStyle(color: AppTheme.textSecondary, fontSize: 15)),
                      ]),
                    )
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) => _RefundCard(
                        request: _requests[i],
                        role: role,
                        onRespond: (action) => _respond(_requests[i], action, role),
                      ),
                    ),
    );
  }
...
class _RefundCard extends StatelessWidget {
  ...
  const _RefundCard({required this.request, required this.role, required this.onRespond});

  Color _statusColor(String status) {
    switch (status) {
      case 'pending_seller': return AppTheme.warning;
      case 'approved_by_seller':
      case 'approved_by_admin':
      case 'refunded': return AppTheme.success;
      case 'rejected_by_seller':
      case 'rejected_by_admin': return AppTheme.error;
      case 'escalated': return AppTheme.accentBlue;
  ...
  Widget build(BuildContext context) {
    ...
    final statusDisplay = request['status_display']?.toString() ?? status;
    ...
    final date = request['created_at']?.toString().substring(0, 10) ?? '';
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(orderNo, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: AppTheme.textPrimary)),
              ),
              Container(
          ...
          if (role != 'buyer')
            Text('Buyer: $buyerName', style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
          Text('ZMW $amount · $date', style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
          const SizedBox(height: 8),
          Text('Reason: $reason', style: const TextStyle(fontSize: 13, color: AppTheme.textPrimary, height: 1.35)),
          if (sellerNote.isNotEmpty) ...[
            const SizedBox(height: 6),
            Text('Seller note: $sellerNote', style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary, height: 1.3)),
          ],
          if (adminNote.isNotEmpty) ...[
            const SizedBox(height: 4),
            Text('Admin note: $adminNote', style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary, height: 1.3)),
          ],
